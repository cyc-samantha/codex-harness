...
description: "Analyzes accumulated pipeline observations and cost metrics to recommend per-role model downgrades/upgrades when outcomes are statistically indistinguishable."
...
```bash
python3 .agents/skills/harness-eval-model-effectiveness/analyze.py [--project-hash <hash>] [--out <path>]
```
